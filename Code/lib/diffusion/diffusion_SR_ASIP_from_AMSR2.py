...
fusion_ckpt  = torch.load(FUSION_CKPT, map_location=device, weights_only=False)
fusion_model = mdl(
...
est1_np  = np.where(plot_mask, np.nan, np.clip(x_estim[idx,  0].detach().cpu().numpy()*100, 0, 100)) # Clip prediction back to [0,100] range for plotting
est2_np  = np.where(plot_mask, np.nan, np.clip(x_estim2[idx, 0].detach().cpu().numpy()*100, 0, 100)) # Clip prediction back to [0,100] range for plotting
