...
'''python daily_composite.py 2020-03-01 2020-03-15'''
...
def process_one_day(date, shp):
    """Runs the composite pipeline for a single date and saves the plot.
    Returns True if a plot was produced, False if there was nothing to do."""
    date_str = date.strftime("%Y-%m-%d")
    swath_dir = SWATH_ROOT / f"{date.year:04d}" / f"{date.month:02d}" / f"{date.day:02d}"

    if not swath_dir.exists():
        print(f"  {date_str}: {swath_dir} does not exist — skipping")
        return False

    swath_files = sorted(swath_dir.glob('*resampled.nc'), key=parse_swath_time)
    print(f"  {date_str}: found {len(swath_files)} swath file(s)")

    if len(swath_files) == 0:
        print(f"  {date_str}: no swath files — skipping")
        return False

    out_path = OUTPUT_DIR / f"SPICE_SIC_full_{date_str}.png"
    if out_path.exists():
    ...
    sic_composite = np.where(sic_count > 0, sic_sum / np.maximum(sic_count, 1), np.nan)
    print(f"  {date_str}: all swaths processed")
    ...
    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.set_title(f'{date_str}', fontsize=12)
    plt.tight_layout()
    ...
    print(f"  {date_str}: saved {out_path}")
    return True


def main():
    parser = argparse.ArgumentParser(description="Run SPICE SIC composite for each day in a date range and save as PNG.")
    ...
    # ── Load model once, reused across all days ──────────────────────────────────
    global model
    ckpt_path = os.path.join(CKPT_DIR, MODEL_NAME.lower(), f'best_model_{POSTFIX}.pth')
    ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
    model = FusionNetASPP(in_channels=ckpt['in_channels'], features=ckpt['features']).to(device)
    ...
    for date in daterange(start_date, end_date):
        produced = process_one_day(date, shp)
        if produced:
            n_done += 1
        else:
            n_skipped += 1

    print(f"\nFinished {args.start_date} to {args.end_date}. done={n_done}, skipped={n_skipped}")
